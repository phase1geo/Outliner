...
public class BaseMenu {
  ...
  //-------------------------------------------------------------
  // Default constructor
  public BaseMenu( Gtk.Application app, OutlineTable ot, string group ) {
    ...
    ot.win.shortcuts.shortcut_changed.connect( handle_shortcut_change );

    // Create and add the action group to the mindmap canvas
    _group = new SimpleActionGroup();
    _ot.insert_action_group( _group_name, _group );
  ...
  protected void append_menu_item( GLib.Menu menu, KeyCommand command, string label, bool grab_canvas = true ) {
    ...
    var shortcut = _ot.win.shortcuts.get_shortcut( command );
    if( shortcut != null ) {
  ...
  protected void set_enabled( KeyCommand command, bool enable ) {
    var action = (SimpleAction)_group.lookup_action( command.to_string() );
    if( action != null ) {
